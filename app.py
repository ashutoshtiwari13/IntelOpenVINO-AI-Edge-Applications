'''
Created by Ashutosh ,on 6/2
'''
import argparse
import numpy as np
import os
import cv2
from model_selection import output_of_selection, preprocessing
from inference_eng import Network
...
def get_args():
    ...
    parser= argparse.ArgumentParser("Car Edge App with Inference Engine")

    c_desc = "CPU extension file location , if application"
    d_desc= "Device , if not CPU (GPU < FPGA< MYRIAD)"
    i_desc= "Location of input image"
    m_desc= "Location of OpenVINO pre-trained model XML file"
    t_desc= "the type of model :POSE , TEXT or CAR_TYPE_COLOR"
...
def perform_inference(args):
    ...
    ### Note: This will require using `output` to get the correct
    ###       function, and then feeding the output to that function.
    print(args.t)
    output_func = output_of_selection(args.t)
